...
(function() {
  ...
  if (!toggleBtn || !navDesktop) return;
  ...
  document.addEventListener('click', (e) => {
    if (navDesktop.getAttribute('aria-hidden') === 'false') {
      if (!navDesktop.contains(e.target) && !toggleBtn.contains(e.target)) {
        closeMenu();
      }
    }
  });
  ...
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape' && navDesktop.getAttribute('aria-hidden') === 'false') {
      closeMenu();
      toggleBtn.focus();
    }
  });
